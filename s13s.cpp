...
//				}
	...
	int CGameLogic::EnumCards(uint8_t *cards, int& len, int n,
		FetchTy ty, uint8_t(*cardsList)[5], int size) {
		//手牌按牌值从小到大排序(A23...QK)
		//SortCards(cards, len, true, true, true);
		assert(n <= 5);
		int cpylen, loop = len / n;
		bool succ;
		uint8_t /*dst[5] = { 0 },*/ cpy[13] = { 0 };
		int c = 0;
		for (int i = 0; i < loop; ++i) {
			//取牌值相同的n张牌
			succ = FetchCardsFrom(cards, len, ty, cardsList[c]/*dst*/, n, cpy, cpylen);
			if (succ) {
				memset(cards, 0, sizeof(uint8_t)*len);
				memcpy(cards, cpy, cpylen);
				len = cpylen;
				memset(cpy, 0, sizeof(uint8_t)*cpylen);
				cpylen = 0;
				++c;
				printf("牌型%d：", i);
				PrintCardList(cardsList[c]/*dst*/, n);
				printf("余牌%d：", i);
				PrintCardList(cards, len);
				continue;
			}
			break;
		}
	...
	int  CGameLogic::EnumCards(uint8_t *cards, int& len, int n,
		...
		for (int i = 0; i < loop; ++i) {
			//取牌值相同的n张牌
			succ = FetchCardsFrom(cards, len, ty, dst, n, cpy, cpylen);
			if (succ) {
				memset(cards, 0, sizeof(uint8_t)*len);
				memcpy(cards, cpy, cpylen);
				len = cpylen;
				memset(cpy, 0, sizeof(uint8_t)*cpylen);
				cpylen = 0;
				boost::shared_ptr<uint8_t> p(new uint8_t[n+1]);
				for (int i = 0; i < n; ++i) {
					boost::get_pointer(p)[i] = dst[i];
				}
				boost::get_pointer(p)[n] = '\0';
				cardsList.push_back(p);
				++c;
				printf("牌型%d：", i);
				PrintCardList(dst, n);
				printf("余牌%d：", i);
	...
	//枚举组合牌型
	int CGameLogic::EnumCards(uint8_t *cards, int& len, int n, FetchTy ty) {
		//手牌按牌值从小到大排序(A23...QK)
		//SortCards(cards, len, true, true, true);
		int cpylen, loop = len / n;
		bool succ;
		...
		for (int i = 0; i < loop; ++i) {
			//取牌值相同的n张牌
			succ = FetchCardsFrom(cards, len, ty/*SameValue*/, dst, n, cpy, cpylen);
			if (succ) {
				memset(cards, 0, sizeof(uint8_t)*len);
				memcpy(cards, cpy, cpylen);
				len = cpylen;
	...
	//同花顺：花色相同的连续五张牌
	int CGameLogic::EnumShunJin(uint8_t *cards, int& len, uint8_t(*cardsList)[5], int size) {
		int c = 0;
		c = EnumCards(cards, len, 5, SameColorConsecutive, cardsList, size);
 		return c;
	}
	
	//铁支：除了四张值相同的牌外没有其它牌型
	int CGameLogic::EnumTieZhi(uint8_t *cards, int& len, uint8_t(*cardsList)[5], int size) {
		int c = 0;
		c = EnumCards(cards, len, 4, SameValue, cardsList, size);
		return c;
	}
	
	//葫芦：一组三条加上一组对子
	int CGameLogic::EnumHuLu(uint8_t *cards, int& len, uint8_t(*cardsList)[5], int size) {
		int c = 0;
		//三条数
		uint8_t santiaoList[4][5] = {0};
		c = EnumCards(cards, len, 3, SameValue, &santiaoList[0], size);
		//对子数
		uint8_t duiziList[4][5] = { 0 };
		c = EnumCards(cards, len, 2, SameValue, &duiziList[0], size);
		//任意一个三条和对子组合
		return c;
	}
	
	//同花：花色相同的五张牌，非顺子
	int CGameLogic::EnumTongHua(uint8_t *cards, int& len, uint8_t(*cardsList)[5], int size) {
		int c = 0;
		//五张同花数
		uint8_t tonghuaList[4][5] = { 0 };
		c = EnumCards(cards, len, 5, SameColor, &tonghuaList[0], size);
		return c;
	}
	
	//顺子：花色不同的连续五张牌(10JQKA最大，A2345最小)
	int CGameLogic::EnumShunZi(uint8_t *cards, int& len, uint8_t(*cardsList)[5], int size) {
		int c = 0;
		//五张顺子数
		uint8_t shunziList[4][5] = { 0 };
		c = EnumCards(cards, len, 5, Consecutive, &shunziList[0], size);
		return c;
	}
	
	//三条：除了三张值相同的牌外没有其它牌型
	int CGameLogic::EnumThreeTiao(uint8_t *cards, int& len, uint8_t(*cardsList)[5], int size) {
		int c = 0;
		//三条数
		uint8_t santiaoList[4][5] = { 0 };
		c = EnumCards(cards, len, 3, SameValue, &santiaoList[0], size);
	...
	//两对：两个对子加上一张单牌
	int CGameLogic::EnumTwoDui(uint8_t *cards, int& len, uint8_t(*cardsList)[5], int size) {
		int c = 0;
		//对子数
		uint8_t duiziList[4][5] = { 0 };
		c = EnumCards(cards, len, 2, SameValue, &duiziList[0], size);
	...
	int CGameLogic::EnumDuiZi(uint8_t *cards, int& len, uint8_t(*cardsList)[5], int size) {

	}
	
	//枚举组合牌型(四炸/三张/对子/散牌)
	void CGameLogic::EnumCombineCards(uint8_t *cards, int len)
	{
		//手牌按牌值从小到大排序(A23...QK)
		//SortCards(cards, len, true, true, true);
		int c = 0;
		c = EnumCards(cards, len, 4, SameValue);
		c = EnumCards(cards, len, 3, SameValue);
		c = EnumCards(cards, len, 2, SameValue);
	}

	

	//三同花顺：三墩均是同花顺的牌型
	bool CGameLogic::IsThreeShunJin(uint8_t *cards)
	{
		//手牌按牌值从小到大排序(A23...QK)
		//SortCards(cards, MaxCount, true, true, true);
		int len = MaxCount, cpylen;
		bool succ;
		uint8_t dst[5] = { 0 }, cpy[13] = { 0 };
		//取前面的五张牌
		succ = FetchCardsFrom(cards, len, SameColorConsecutive, dst, 5, cpy, cpylen);
		if (succ) {
			memset(cards, 0, sizeof(uint8_t)*len);
			memcpy(cards, cpy, cpylen);
			len = cpylen;
			...
			if (succ) {
				...
				cpylen = 0;
				printf("中墩：");
				PrintCardList(dst, 5);//中墩
				printf("余牌：");
				PrintCardList(cards, len);
				//取后面三张牌
				succ = FetchCardsFrom(cards, len, SameColorConsecutive, dst, 3, cpy, cpylen);
				if (succ) {
					memset(cards, 0, sizeof(uint8_t)*len);
					memcpy(cards, cpy, cpylen);
					len = cpylen;
	...
	bool CGameLogic::IsThreeBomb(uint8_t *cards, int n) {
		int len = MaxCount;
		int c = EnumCards(cards, len, n, SameValue);
		return 3 == c;
	}
	
	//全大：全是8至A的牌型
	bool CGameLogic::IsAllBig(uint8_t *cards) {
		//手牌按牌点从小到大排序(23...QKA)
		SortCards(cards, MaxCount, false, true, true);
		return cards[0] >= 0x08;
	}

	//全小：全是2至8的牌型
	bool CGameLogic::IsAllSmall(uint8_t *cards) {
		//手牌按牌点从小到大排序(23...QKA)
		SortCards(cards, MaxCount, false, true, true);
		return cards[MaxCount-1] <= 0x08;
	}
	
	
	
	//双怪冲三：二对葫芦加上一个对子加上一张单张的牌型
	bool CGameLogic::IsTwoGuaiThree(uint8_t *cards) {
		int len = MaxCount, c = 0;
		//不含炸弹
		c = EnumCards(cards, len, 4, SameValue);
		if (c == 0) {
			//两组三条
	...
	bool CGameLogic::IsFourTaoThree(uint8_t *cards) {
		int len = MaxCount, c = 0;
		//不含炸弹
		c = EnumCards(cards, len, 4, SameValue);
		if (c == 0) {
	...
	bool CGameLogic::IsFiveDuiThree(uint8_t *cards) {
		int len = MaxCount, c = 0;
		//不含炸弹
		c = EnumCards(cards, len, 4, SameValue);
		if (c == 0) {
	...
	//六对半：六个对子加上一张单张的牌型
	bool CGameLogic::IsSixDuiHalf() {
		return c2 == 6;
	}
	
	//三顺子：三墩均是顺子的牌型
	bool CGameLogic::IsThreeShunZi(uint8_t *cards) {
		//手牌按牌值从小到大排序(A23...QK)
		//SortCards(cards, MaxCount, true, true, true);
		int len = MaxCount, cpylen;
		bool succ;
		uint8_t dst[5] = { 0 }, cpy[13] = { 0 };
		//取前面的五张牌
		succ = FetchCardsFrom(cards, len, Consecutive, dst, 5, cpy, cpylen);
		if (succ) {
			memset(cards, 0, sizeof(uint8_t)*len);
			memcpy(cards, cpy, cpylen);
			len = cpylen;
			...
			if (succ) {
				...
				PrintCardList(cards, len);
				//取后面三张牌
				succ = FetchCardsFrom(cards, len, Consecutive, dst, 3, cpy, cpylen);
				if (succ) {
					memset(cards, 0, sizeof(uint8_t)*len);
					memcpy(cards, cpy, cpylen);
					len = cpylen;
	...
	//三同花：三墩均是同一花色的牌型
	bool CGameLogic::IsThreeTongHua(uint8_t *cards) {
		//手牌按花色升序(方块到黑桃)，同花色按牌值从小到大排序
		SortCardsColor(cards, MaxCount, true, true, true);
		int len = MaxCount, cpylen;
		bool succ;
		uint8_t dst[5] = { 0 }, cpy[13] = { 0 };
		//取前面的五张牌
		succ = FetchCardsFrom(cards, len, SameColor, dst, 5, cpy, cpylen);
		if (succ) {
			memset(cards, 0, sizeof(uint8_t)*len);
			memcpy(cards, cpy, cpylen);
			len = cpylen;
			...
			if (succ) {
				...
				PrintCardList(cards, len);
				//取后面三张牌
				succ = FetchCardsFrom(cards, len, SameColor, dst, 3, cpy, cpylen);
				if (succ) {
					memset(cards, 0, sizeof(uint8_t)*len);
					memcpy(cards, cpy, cpylen);
					len = cpylen;
